...
amenities_query <- opq(location) %>%
  add_osm_feature(key = "amenity", value = c("restaurant", "supermarket")) 
```
...
```{r}
st_write(amenities, "amenities.shp", delete_dsn = TRUE)
```
...
ggplot() +
  ...
  geom_sf(data = amenities_data$osm_points, aes(color = amenity), size = 1) +
  theme_minimal() +
