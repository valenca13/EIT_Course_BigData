...
Data collection began in _February 2020_ and ended in _June 2025_.
...
* CSV file
```{r}
data_csv <- read.csv("Data/Automatic_Hourly_Pedestrian_Count.csv")
```
...
## Convert dataset into _dataframe_

It is good practice to: 
* convert the database into dataframe, as many packages only work with dataframes.
* To not work on the raw dataset. 
...
> **Note:** Google Maps uses _WGS84_, which corresponds to EPSG = 4326
...
# plot counts per day and counter

```{r}
library(ggplot2)

ggplot(df_sf, aes(x = Date, y = TotalCount, color = Location_code)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  labs(
    title = "Total Count per Location Over Time",
    x = "Time",
    y = "Total Count",
    color = "Location"
  ) +
